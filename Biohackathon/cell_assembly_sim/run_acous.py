...
def main():
    ...
    parser.add_argument('--output-dir', default=None,
                        help='Directory for generated figures (default: system temporary directory)')
    ...
    # Default to the output_v2 folder in the same directory as this script
    ...
    kw = dict(
        n_cells=50,
        init_spread=0.015,      # 20 mm std — spread across field
        merge_distance=0.004,   # 3 mm merge threshold
        dt=0.001,
    ...
    for short, title, sim, metrics in results:
        ...
        # A 32-minute experiment compressed 120x produces a 16-second GIF.
        animation_path = os.path.join(output_dir, f"{short}_animation.gif")
        save_animation_mpl(
            sim.snapshots, title=title, save_path=animation_path,
            bounds=kw['bounds'], duration=16.0, fps=20,
        )
        print(f"  Saved {animation_path} [16 s]")
