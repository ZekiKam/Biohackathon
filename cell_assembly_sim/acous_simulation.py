...
class CellAssemblySimulation:
    ...
    * Sphere trap   : Fibonacci-sphere hologram for spheroid assembly
    ...
    def __init__(self, activations, board, n_cells=50,
                 init_spread=0.015, merge_distance=0.004,
                 dt=0.001, bounds=0.04, max_step=0.001,
...
def save_animation_mpl(snapshots, title, save_path, bounds=0.04,
                       duration=16.0, fps=20):
    """Save interpolated simulation snapshots as a timed 3D GIF."""
    if len(snapshots) < 2:
        raise ValueError("At least two snapshots are required for animation")
    if duration <= 0 or fps <= 0:
        raise ValueError("duration and fps must be positive")

    steps = np.asarray([snapshot[0] for snapshot in snapshots], dtype=float)
    positions = np.asarray([snapshot[1] for snapshot in snapshots])
    cluster_ids = [snapshot[2] for snapshot in snapshots]
    n_frames = max(2, round(duration * fps))
    frame_steps = np.linspace(steps[0], steps[-1], n_frames)
    ...
    cmap = plt.cm.get_cmap('tab20', max(len(np.unique(cluster_ids[0])), 2))
    scatter = ax.scatter([], [], [], s=45, alpha=0.85,
                         edgecolors='k', linewidth=0.25)

    def update(frame):
        step = frame_steps[frame]
        right = min(np.searchsorted(steps, step, side='right'), len(steps) - 1)
        left = max(right - 1, 0)
        if right == left:
            current = positions[left]
        else:
            weight = (step - steps[left]) / (steps[right] - steps[left])
            current = ((1.0 - weight) * positions[left]
                       + weight * positions[right])

        # Use the nearest recorded cluster assignment for stable colours.
        nearest = left if step - steps[left] <= steps[right] - step else right
        colors = [cmap(int(cid) % 20) for cid in cluster_ids[nearest]]
        scatter._offsets3d = (current[:, 0] * 1000,
                              current[:, 1] * 1000,
                              current[:, 2] * 1000)
        scatter.set_color(colors)
        ax.set_title(f'{title}\nStep {step:.0f} / {steps[-1]:.0f}')
        return (scatter,)
